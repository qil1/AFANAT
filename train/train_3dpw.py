...
if __name__ == '__main__':
    ...
    val_dataset = Datasets(config, split=1)
    print('>>> Validation dataset length: {:d}'.format(val_dataset.__len__()))
    val_loader = DataLoader(val_dataset, batch_size=config.test_batch_size, shuffle=False, num_workers=0,
                            pin_memory=True)
    ...
    if config.iter > 0:
        cp_path = config.model_path % (config.save_name, config.iter - 1)
