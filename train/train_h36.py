...
def val_func(epoch, test=False):
    ...
    for act in ACTION:
        ...
        for (gt3d, _) in tqdm(loader[act]):
            ...
            pred32 = pred32.reshape([-1, config.t_pred, 32, 3])
    ...
    losses_str[16] = losses_str[16].join(
        "{} | {:<6.4f} | {:<6.4f} | {:<6.4f} | {:<6.4f} | {:<6.4f} | {:<6.4f}".format("Average", avg_mpjpe1 / 15.,
                                                                                      avg_mpjpe2 / 15.,
                                                                                      avg_mpjpe3 / 15.,
                                                                                      avg_mpjpe4 / 15.,
                                                                                      avg_mpjpe5 / 15.,
                                                                                      avg_mpjpe6 / 15.))
    losses_str[17] = losses_str[17].join("====================================================")
    ...
    for act in ACTION:
        ...
        for k in range(len(action_mpjpe['walking'])):
...
if __name__ == '__main__':
    ...
    val_data_loader = {}
    for act in ACTION:
        dataset = Datasets(opt=config, split=1, actions=act)
        val_tot += len(dataset)
        val_data_loader[act] = DataLoader(dataset, batch_size=config.test_batch_size, shuffle=False, num_workers=0,
                                          pin_memory=True)
    print('>>> Validation dataset length: {:d}'.format(val_tot))
    test_tot = 0
    test_data_loader = {}
    for act in ACTION:
        dataset = Datasets(opt=config, split=2, actions=act)
        test_tot += len(dataset)
