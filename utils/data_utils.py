...
def rotmat2euler_torch(opt, R):
    ...
    if len(idx_spec2) > 0:
        R_spec2 = R[idx_spec2, :, :]
        eul_spec2 = torch.zeros(len(idx_spec2), 3).float().to(opt.gpu_index)
        eul_spec2[:, 2] = 0
        eul_spec2[:, 1] = np.pi / 2
    ...
    if len(idx_remain) > 0:
        ...
        eul_remain = torch.zeros(len(idx_remain), 3).float().to(opt.gpu_index)
...
def expmap2rotmat_torch(opt, r):
    ...
    n = r1.data.shape[0]
    R = torch.eye(3, 3).repeat(n, 1, 1).float().to(opt.gpu_index) + torch.mul(
